...
function sortBooks(books: AbsBook[], sortBy: BookSortOption): AbsBook[] {
  return [...books].sort((left, right) => {
    if (sortBy === 'author') {
      return compareText(left.authorName, right.authorName) || compareText(left.title, right.title)
    }
    ...
    return compareText(left.title, right.title) || compareText(left.authorName, right.authorName)
  })
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 px-4 py-6 backdrop-blur-sm">
      <div
        ...
        aria-modal="true"
        className="flex h-[min(740px,calc(100vh-32px))] w-full max-w-6xl flex-col overflow-hidden rounded-[24px] border border-[#452020] bg-[linear-gradient(180deg,#150808_0%,#0d0404_100%)] shadow-[0_30px_90px_rgba(0,0,0,0.55)]"
        role="dialog"
      >
        <div className="flex flex-none items-center justify-between gap-4 border-b border-[#351616] px-5 py-3.5">
          <div className="min-w-[13rem] flex-none">
            <div className="text-[11px] font-semibold uppercase tracking-[0.24em] text-[#b78787]">
              Source Browser
            </div>
            <h2 id="abs-library-title" className="mt-1.5 text-xl font-semibold text-[#fff4f4]">
          ...
          <div
            ...
            data-testid="source-browser-toolbar"
          >
            <label className="min-w-0 max-w-[190px] flex-1">
              <span className="sr-only">Audiobookshelf library</span>
              <select
                aria-label="Audiobookshelf library"
                className="h-9 w-full rounded-full border border-[#4b2222] bg-[#170909] px-3 text-xs font-medium text-[#f0d0d0] outline-none transition-colors focus:border-[#dc2626] disabled:opacity-50"
                disabled={loading || absLibrary.libraries.length === 0}
                onChange={(event) => setSelectedLibraryId(event.target.value)}
                value={selectedLibraryId ?? ''}
              >
                {absLibrary.libraries.length === 0 && (
                  <option value="">{loading ? 'Loading libraries...' : 'No libraries'}</option>
                )}
            ...
            {multiSelectEnabled && (
              <button
                className="h-9 rounded-full border border-[#8a2d2d] bg-[#2a0f0f] px-3 text-xs font-medium text-[#fff0f0] shadow-[0_8px_18px_rgba(120,20,20,0.16)] transition-colors hover:border-[#dc2626] hover:bg-[#341212] hover:text-white disabled:cursor-not-allowed disabled:opacity-50"
            ...
            <button
              className="h-9 rounded-full border border-[#4b2222] px-3 text-xs text-[#f0d0d0] transition-colors hover:border-[#dc2626] hover:text-[#fff4f4] disabled:cursor-not-allowed disabled:opacity-50"
              disabled={loading || !settings.absUrl}
            ...
            <button
              className="h-9 rounded-full border border-[#4b2222] px-3 text-xs text-[#f0d0d0] transition-colors hover:border-[#dc2626] hover:text-[#fff4f4]"
        ...
        <div className="flex-none border-b border-[#351616] px-5 py-3">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end">
            <label className="flex-1">
              <div className="mb-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#b78787]">
                Search
              </div>
              <input
                className="h-10 w-full rounded-[14px] border border-[#482020] bg-[#170909] px-3 text-sm text-[#fff4f4] outline-none transition-colors placeholder:text-[#8c5d5d] focus:border-[#dc2626]"
            ...
            <label className="w-full lg:w-[240px]">
              ...
              <select
                className="h-10 w-full rounded-[14px] border border-[#482020] bg-[#170909] px-3 text-sm text-[#fff4f4] outline-none transition-colors focus:border-[#dc2626]"
          ...
          <div className="mt-2 flex h-5 flex-wrap items-center justify-between gap-3 overflow-hidden text-[10px] uppercase tracking-[0.16em] text-[#9f7373]">
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        ...
                        <div
                          className="mt-1 h-5 truncate text-xs leading-5 text-[#d4afaf]"
                    ...
                    <div className="mt-3 border-t border-[#341616] pt-3">
                      <div className="flex items-center justify-between gap-3">
                        ...
                        <span className="text-sm text-[#f2d6d6]">
